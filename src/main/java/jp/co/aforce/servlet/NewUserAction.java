...
public class NewUserAction extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (result == 1) {
				// 登録成功
				response.sendRedirect(request.getContextPath() + "/views/register-success.jsp");
			} else {
				// 登録失敗（ID重複など）
				response.sendRedirect(request.getContextPath() + "/views/register-error.jsp");
			}
